...
class digit_gen {
  ...
  async load_gens() {
    this.model =
      await tf.loadLayersModel('file:///home/douglas/Playground/Clock/nodetest/models/js_models/'+this.digit+'/model.json');
...
class clock_gen {
  ...
  update_clock() {
    ...
    setTimeout(this.update_clock.bind(this), 1000 * (62 - date.getSeconds()));
  }
};

var clock = new clock_gen();

const server = http.createServer((req, res) => {
  res.statusCode = 200;
  res.setHeader('Content-Type', 'text/html');
  res.end('<img src="'+clock.png+'"/>');
});
